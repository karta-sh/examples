...
**Kriya** is the agent that answers on the [karta.sh](https://karta.sh) home page.
...
- **`CLAUDE.md`** - Kriya's whole personality and rules. Treated as
...
- **`homepage-conversation.md`** - the scripted intro the visitor reads on the
  home page before they type. Kriya treats it as a conversation it has already
...
stream Kriya's replies into the page's own textbox and content area. The visitor
reads the scripted intro, and their first message is Kriya's first *real* turn -
...
The live deployment (the `org-8z06atvr/kriya` project) and its origin-gated,
credit-capped `pk_live_` embed key are managed by the Karta team - the key
allowlists `karta.sh` and `localhost` (see RFC 0020 EX-7). A vanity project slug
may replace `org-8z06atvr` before public launch (RFC 0020 OQ6).

## Run it locally

```bash
karta dev --dir apps/claude-code/kriya --port 4747
```
